...
then
    ...
    cat $output/3_httprobe.txt >> /root/httprobe_all.txt ; mkdir $output/root ; var=${var/domains_urlwatch.txt/}; cp $var*.txt $output/root ; cp /root/watch/* $output/root ; cd $output;date|sed -e 's/\ /\_/g'|sed -e 's/\://g' > /root/time.txt;slash=/;dayzoom=`cat /root/time.txt`;addname=_domains.txt;zip=.zip;fname=$dayzoom$addname$zip;zip -q -r $fname * ; fname=$slash$dayzoom$addname$zip ; mv $output$fname /root/zip; curl -X POST -H "Content-type:application/json" --data '{"text":"1"}'  https://hooks.sla.com/services/TM26L9ZEE/BM78UTLGH/wkGCgWEa5Fs4ebeC7asZZXRR
...
then
    ...
    cat $output/3_httprobe.txt >> /root/httprobe_all.txt ; mkdir $output/root ;var=${var/domains_urlwatch.txt/} ;cp $var*.txt $output/root ; cp /root/watch/* $output/root  ; cd $output;date|sed -e 's/\ /\_/g'|sed -e 's/\://g' > /root/time.txt;slash=/;dayzoom=`cat /root/time.txt`;addname=_wildcards.txt;zip=.zip;fname=$dayzoom$addname$zip;zip -q -r $fname * ; fname=$slash$dayzoom$addname$zip ; mv $output$fname /root/zip; curl -X POST -H "Content-type:application/json" --data '{"text":"2"}'  https://hooks.sla.com/services/TM26L9ZEE/BM78UTLGH/wkGCgWEa5Fs4ebeC7asZZXRR
    #var切片结束
    done
    rm /root/var.txt
...
#    done
#    cat $output/5_dir/*.txt > $output/5_dir_all.txt ; sort $output/5_dir_all.txt|uniq|tee sort.txt ; > $output/5_dir_all.txt ; cat sort.txt > $output/5_dir_all.txt ; rm sort.txt ; cat $output/5_dir/*.haha > $output/5_dir/5_301_403.txt ; sort $output/5_dir/5_301_403.txt|uniq|tee sort.txt ; > $output/5_dir/5_301_403.txt ; cat sort.txt > $output/5_dir/5_301_403.txt ; rm sort.txt ; cd $output/5_dir ;  rename 's/\.haha/\.txt/'  ./*
#    cat $output/3_httprobe.txt >> /root/httprobe_all.txt ; mkdir $output/root ; var=${var/domains_urlwatch.txt/} ;cp $var*.txt $output/root ; cp /root/watch/* $output/root ; cd $output;date|sed -e 's/\ /\_/g'|sed -e 's/\://g' > /root/time.txt;slash=/;dayzoom=`cat /root/time.txt`;addname=_monis.txt;zip=.zip;fname=$dayzoom$addname$zip;zip -q -r $fname * ; fname=$slash$dayzoom$addname$zip ; mv $output$fname /root/zip; curl -X POST -H "Content-type:application/json" --data '{"text":"3"}'  https://hooks.sla.com/services/TM26L9ZEE/BM78UTLGH/wkGCgWEa5Fs4ebeC7asZZXRR
#
...
rm /root/httprobe_all.txt ; mkdir $output/root ; var=${var/target.txt/} ;cp $var*.txt $output/root ; cp /root/watch/* $output/root ; cd $output;date|sed -e 's/\ /\_/g'|sed -e 's/\://g' > /root/time.txt;slash=/;dayzoom=`cat /root/time.txt`;addname=_scan.txt;zip=.zip;fname=$dayzoom$addname$zip;zip -q -r $fname *;fname=$slash$dayzoom$addname$zip ; mv $output$fname /root/zip ; curl -X POST -H "Content-type:application/json" --data '{"text":"scan"}'  https://hooks.sla.com/services/TM26L9ZEE/BM78UTLGH/wkGCgWEa5Fs4ebeC7asZZXRR
